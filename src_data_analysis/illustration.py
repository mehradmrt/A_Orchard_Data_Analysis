...
DOY = [159, 174, 189, 196, 211, 215, 243]
orchard = 'Almond'
...
main.info()
mdf_plt = main[['NDRE','NDVI','T_c','SWP']]

#%%
def plot_multi_swp_vs_columns(dataframe, x_columns, y_column):
    plt.figure(figsize=(8, 15), dpi=100)
    axis_limits = {'T_c': (20, 50), 'NDRE': (0, 0.3), 'NDVI': (0.3, 1), 'SWP': (0, 40)}
    labels = {'T_c': '$T_c\ [^\circ C]$', 'NDRE': 'NDRE', 'NDVI': 'NDVI', 'SWP': '|$\psi$| $_{[bar]}$'}
    ...
    for i, x_col in enumerate(x_columns):
        ...
        text_x = ax.get_xlim()[0] + 0.0 * (ax.get_xlim()[1] - ax.get_xlim()[0])
        text_y = ax.get_ylim()[0] - 0.0 * (ax.get_ylim()[1] - ax.get_ylim()[0])
...
def plot_weather_parameters(df, DOY):
    ...
    plt.tight_layout()
    plt.savefig('../figures/'+orchard+'_weather.png',dpi=300)
    plt.show()
...
def swp_extract():
    ...
    plt.title('Almond Orchard', fontsize=16)
    # plt.legend(['stressed','no stress','over watered'])
    plt.xlabel('Day of Year', fontsize = 14)
    plt.ylabel('SWP', fontsize = 14)
    plt.show
    plt.savefig('Almond.png')
...
def pertree(idx_type,i):
    ...
    ax1.errorbar(x, mn, std, fmt='o', color ='black' , linewidth=1, capsize=6)
    ax1.plot(x, mn, color = 'black', linewidth=2)
    ...
    ax1.set_ylabel('{} Pixels Mean'.format(idx_type), color='k')
...
#%%
#### Results Per Tree leaftemp and Indexes
def pertree(idx_type,i):
    mn = list([])
    avg_im = list([])
    std = list([])
    ...
    for j in range(len(testdic)):
        ...
        lt_av.append(lt[lt['test_number']==testdic[j]]['leaf_temp'][swpidx[j]])
        lt_std.append(lt[lt['test_number']==testdic[j]]['STD'][swpidx[j]])
    ...
    ax1.errorbar(x, mn, std, fmt='o', color ='black' , linewidth=1, capsize=6)
    ax1.plot(x, mn, color = 'black', linewidth=2)
    ax2.errorbar(x, lt_av, lt_std, fmt='o', color ='red', linewidth=1, capsize=6)
    ax2.plot(x, lt_av, color = 'red' , linewidth=2)

    ax1.set_xlabel('Test Day Number')
    ax1.set_ylabel('{} Pixels Median'.format(idx_type), color='k')
    ax2.set_ylabel('Leaf Temp Average', color='r')
    plt.title('Tree {}'.format(i+1))
    plt.show()
...
def swp_lt(i):
    ...
    plt.title('Almond Tree {}'.format(i))
    plt.show()
...
plt.title('Almond Trees per Test Day')    
plt.show()
